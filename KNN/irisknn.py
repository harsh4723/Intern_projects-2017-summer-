...
def confusionmatrix(testvalue,testdata):
	...
	CM=confusion_matrix(actualvalue,testvalue,labels=["Iris-setosa","Iris-virginica","Iris-versicolor"])
	print CM
	sumoftrues=0
	f=0
	for x in CM:
	...
	s1=CM[0][0]+CM[1][0]+CM[2][0]
	print 'predicts iris setosa how often it is correct '+repr(float(CM[0][0])/float(s1))
	s2=CM[0][1]+CM[1][1]+CM[2][1]
	print 'predicts iris virginica how often it is correct '+repr(float(CM[1][1])/float(s2))
	s3=CM[0][2]+CM[1][2]+CM[2][2]
	print 'predicts iris versicolor how often it correct '+repr(float(CM[2][2])/float(s3))
